...
values_update = ("Dysentry", "PT0010")

query_select = "select %s from patients where patient_id = %s;"
values_select = ("present_complaint", "PT0010")

if graph:
    update_result = graph.invoke({"query": query_update, "values": values_update, "response": [{}]})
    select_result = graph.invoke({"query": query_select, "values": values_select, "response": [{}]})
...
with open("update_agent.png", "wb") as f:
    f.write(png_data)
